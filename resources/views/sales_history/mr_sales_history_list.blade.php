                        ...
                        <form method="post" action="{{ route('admin.salesHistoryList') }}">
                            ...
                            @if(isset($filter) && ($filter == 1))
                                <div class="col-md-1 mt-4" style="margin-left: 2%;margin-top: 1.3rem!important">
                                    <a href="{{ route('admin.salesHistoryList') }}" class="btn btn-danger mt-1 cancel_button" id="filter" name="save_and_list" value="save_and_list" style="margin-left: -100px;">Reset</a>
                                </div>
                            @endif
                            </div>
                        </form>
                            ...
                            @if(!is_null($get_sales_data))
                                @foreach($get_sales_data as $gk => $gv)
                                    <tr>
                                        ...
                                        <td class="status_{{$gv->id}}">{{ $gv->is_submited == 1 ? 'Submitted' : 'Pending' }}</td>
                                        @if($gv->is_submited == '1')
                                        <td>
                                            <select class="form-control confirm_status" data-id="{{$gv->id}}" data-confirm="{{ Auth::guard()->user()->id }}">
                                        ...
                                        <td>
                                            <a class="btn btn-primary waves-effect waves-light save_and_next_button" href="{{ route('admin.mrHistoryReportList',$gv->id) }}" role="button" title="Stockiest Monthly Sales"><i class="bx bx-pencil"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                            @endif
